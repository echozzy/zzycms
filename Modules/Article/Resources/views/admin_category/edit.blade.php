...
@section('content')
    @component('admin::components.main',['title'=>"编辑文章分类[$adminCategory[cat_name]]",'body_css'=>'col-sm-9 m-auto'])
        @slot('breadcrumb')
        ...
        @slot('nav')
            <li class="nav-item">
                <a pjax class="nav-link" href="/article/adminCategory">文章分类</a>
            </li>
            <li class="nav-item">
                <a class="nav-link active" href="#">编辑文章分类</a>
            </li>
        @endslot
        @slot('body')
            <form action="/article/adminCategory/{{$adminCategory['id']}}" method="post">
                @csrf
                @method('PUT')
                <div class="card-body">
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <select id="p_id" name="p_id" class="form-control">
                                ...
                                <option value="{{$item['id']}}" {{$item['_selected']?'selected':''}} {{$item['_disabled']?'disabled':''}} >{!!$item['_cat_name']!!}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            ...
                            value="{{$adminCategory['cat_description']}}">
                        </div>
                    </div>
                </div>
